...
contract GrandpaFloki is ERC20, Ownable {
    ...
    uint256 public totalFees =
        flokiDividendRewardsFee
            .add(marketingFee)
            .add(flojrDividendRewardsFee)
            .add(liquidityFee);
    ...
    event UpdateflokiDividendTracker(
        address indexed newAddress,
        address indexed oldAddress
    );
    event UpdateflojrDividendTracker(
        address indexed newAddress,
        address indexed oldAddress
    );
    ...
    function updateMarketingWallet(address _newWallet) external onlyOwner {
        require(
            ...
            "GrandpaFloki: The marketing wallet is already this address"
        );
    ...
    function setFlokiDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = flokiDividendRewardsFee
                .add(marketingFee)
                .add(flojrDividendRewardsFee)
                .add(liquidityFee);
            flokiDividendEnabled = _enabled;
    ...
    function setFlojrDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = flojrDividendRewardsFee
                .add(marketingFee)
                .add(flokiDividendRewardsFee)
                .add(liquidityFee);
            flojrDividendEnabled = _enabled;
        }
    ...
    function updateflokiDividendTracker(address newAddress) external onlyOwner {
        require(
            newAddress != address(flokiDividendTracker),
            "GrandpaFloki: The dividend tracker already has that address"
        );

        FLOKIDividendTracker newflokiDividendTracker = FLOKIDividendTracker(
            payable(newAddress)
        );

        require(
            newflokiDividendTracker.owner() == address(this),
            "GrandpaFloki: The new dividend tracker must be owned by the GrandpaFloki token contract"
        );

        newflokiDividendTracker.excludeFromDividends(
            address(newflokiDividendTracker)
        );
        ...
        emit UpdateflokiDividendTracker(
            newAddress,
            address(flokiDividendTracker)
        );
    ...
    function updateflojrDividendTracker(address newAddress) external onlyOwner {
        require(
            newAddress != address(flojrDividendTracker),
            "GrandpaFloki: The dividend tracker already has that address"
        );

        FLOJRDividendTracker newflojrDividendTracker = FLOJRDividendTracker(
            payable(newAddress)
        );

        require(
            newflojrDividendTracker.owner() == address(this),
            "GrandpaFloki: The new dividend tracker must be owned by the GrandpaFloki token contract"
        );

        newflojrDividendTracker.excludeFromDividends(
            address(newflojrDividendTracker)
        );
        ...
        emit UpdateflojrDividendTracker(
            newAddress,
            address(flojrDividendTracker)
        );
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(
            ...
            "GrandpaFloki: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "GrandpaFloki: Account is already exluded from fees"
        );
        isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "GrandpaFloki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "GrandpaFloki: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "GrandpaFloki: Cannot update gasForProcessing to same value"
        );
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance)
        ...
        onlyOwner
    {
        flokiDividendTracker.updateMinimumTokenBalanceForDividends(
            newMinimumBalance
        );
        flojrDividendTracker.updateMinimumTokenBalanceForDividends(
            newMinimumBalance
        );
    }
    ...
    function getTotalFlokiDividendsDistributed()
        external
        view
        returns (uint256)
    {
    ...
    function getTotalFlojrDividendsDistributed()
        external
        view
        returns (uint256)
    {
    ...
    function withdrawableFlokiDividendOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function withdrawableFlojrDividendOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function flokiDividendTokenBalanceOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function flojrDividendTokenBalanceOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        (
            uint256 FlokiIterations,
            uint256 FlokiClaims,
            uint256 FlokiLastProcessedIndex
        ) = flokiDividendTracker.process(gas);
        emit ProcessedflokiDividendTracker(
            FlokiIterations,
            FlokiClaims,
            FlokiLastProcessedIndex,
            false,
            gas,
            tx.origin
        );
        ...
        (
            uint256 flojrIterations,
            uint256 flojrClaims,
            uint256 flojrLastProcessedIndex
        ) = flojrDividendTracker.process(gas);
        emit ProcessedflojrDividendTracker(
            flojrIterations,
            flojrClaims,
            flojrLastProcessedIndex,
            false,
            gas,
            tx.origin
        );
    ...
    function getLastFlokiDividendProcessedIndex()
        external
        view
        returns (uint256)
    {
    ...
    function getLastFlojrDividendProcessedIndex()
        external
        view
        returns (uint256)
    {
    ...
    function getNumberOfFlokiDividendTokenHolders()
        external
        view
        returns (uint256)
    {
    ...
    function getNumberOfFlojrDividendTokenHolders()
        external
        view
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        if (!swapping && canSwap && from != uniswapV2Pair) {
            ...
            if (flokiDividendEnabled) {
                uint256 FlokiTokens = contractTokenBalance.div(totalFees).mul(
                    flokiDividendRewardsFee
                );
            ...
            if (flojrDividendEnabled) {
                uint256 flojrTokens = contractTokenBalance.div(totalFees).mul(
                    flojrDividendRewardsFee
                );
        ...
        try
            flokiDividendTracker.setBalance(payable(from), balanceOf(from))
        {} catch {}
        try
            flojrDividendTracker.setBalance(payable(from), balanceOf(from))
        {} catch {}
        try
            flokiDividendTracker.setBalance(payable(to), balanceOf(to))
        {} catch {}
        try
            flojrDividendTracker.setBalance(payable(to), balanceOf(to))
        {} catch {}

        if (!swapping) {
            uint256 gas = gasForProcessing;

            try flokiDividendTracker.process(gas) returns (
                uint256 iterations,
                uint256 claims,
                uint256 lastProcessedIndex
            ) {
            ...
            try flojrDividendTracker.process(gas) returns (
                uint256 iterations,
                uint256 claims,
                uint256 lastProcessedIndex
            ) {
    ...
    function swapAndSendFlokiDividends(uint256 tokens) private {
        ...
        uint256 flokiDividends = IERC20(flokiDividendToken).balanceOf(
            address(this)
        );
        transferDividends(
            flokiDividendToken,
            address(flokiDividendTracker),
            flokiDividendTracker,
            flokiDividends
        );
    }
    ...
    function swapAndSendFlojrDividends(uint256 tokens) private {
        ...
        uint256 flojrDividends = IERC20(flojrDividendToken).balanceOf(
            address(this)
        );
        transferDividends(
            flojrDividendToken,
            address(flojrDividendTracker),
            flojrDividendTracker,
            flojrDividends
        );
    }
...
contract FLOKIDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor()
        DividendPayingToken(
            "GrandpaFloki_Floki_Dividend_Tracker",
            "GrandpaFloki_Floki_Dividend_Tracker",
    ...
    function _transfer(
        ...
        require(
            false,
            "GrandpaFloki_Floki_Dividend_Tracker: No transfers allowed"
        );
    }
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "GrandpaFloki_Floki_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GrandpaFloki contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "GrandpaFloki_Floki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "GrandpaFloki_Floki_Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract FLOJRDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor()
        DividendPayingToken(
            "GrandpaFloki_Flojr_Dividend_Tracker",
            "GrandpaFloki_Flojr_Dividend_Tracker",
            0x338A09a17a7DA2E5c5a6B22344f3a49904224C79
        )
    {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 2000000 * (10**18); //must hold 2000000+ tokens
    ...
    function _transfer(
        ...
        require(
            false,
            "GrandpaFloki_Flojr_Dividend_Tracker: No transfers allowed"
        );
    }
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "GrandpaFloki_Flojr_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GrandpaFloki contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            newClaimWait >= 1800 && newClaimWait <= 86400,
            "GrandpaFloki_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "GrandpaFloki_Flojr_Dividend_Tracker: Cannot update claimWait to same value"
        );
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }
